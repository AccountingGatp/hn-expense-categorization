!(function () {
  ...
  const IMPORT_STATUS_KEY = "hnBankImportedTill.v1";

  function qboAccountName(account) {
    if (!account) return "";
    return String(account.qboAccount || account.label || account.name || "").trim();
  }

  function ensureCcAccountColumn(columns) {
    const out = (columns || []).slice();
    const has = out.some((c) => {
      const n = String(c || "").trim().toLowerCase();
      return n === "cc account" || n === "payment account" || n === "bank";
    });
    if (!has) {
      const dateIdx = out.findIndex((c) => String(c || "").trim().toLowerCase() === "date");
      out.splice(dateIdx >= 0 ? dateIdx + 1 : 0, 0, "CC Account");
    }
    return out;
  }

  function statusKey(account) {
    return String((account && account.code) || "") + "|" + String((account && account.name) || "");
  }

  function loadImportStatus() {
    try {
      const v = JSON.parse(localStorage.getItem(IMPORT_STATUS_KEY) || "{}");
  ...
  function saveImportedTill(account, dateObj) {
    if (!account || !dateObj) return;
    const all = loadImportStatus();
    all[statusKey(account)] = ExpCore.fmtDate(dateObj);
    try { localStorage.setItem(IMPORT_STATUS_KEY, JSON.stringify(all)); } catch (e) {}
  }

  function renderImportStatus() {
    const el = t("importStatusTable");
    if (!el) return;
    const status = loadImportStatus();
    let html = '<thead><tr><th>Bank Name</th><th>Imported till</th><th>Books</th></tr></thead><tbody>';
    o.bankAccounts.forEach((acct) => {
      const bankName = acct.type === "Credit Card"
        ? (acct.name === "Capital One" ? "Capital One " + acct.code : "Amex " + acct.code)
        : acct.code;
      html += "<tr><td>" + x(bankName) + "</td><td>" + x(status[statusKey(acct)] || "") + "</td><td>" + x(acct.books || "") + "</td></tr>";
    });
    html += "</tbody>";
    el.innerHTML = html;
  }
  ...
  function d() {
    ...
    const q = window.prompt(
      'QBO CC/Payment Account name (written to every exported row for this statement):',
      a.trim() + (n.trim() ? " " + n.trim() : ""),
    );
    if (null === q) return;
    const books = window.prompt('Books / entity label (optional, e.g. HN, SB, SONANI):', "");
    ...
    })({ code: n.trim(), name: a.trim(), type: "Bank/CC", books: books.trim(), qboAccount: (q || "").trim() || r, label: r }),
      (o.bankAccounts = l()),
      renderImportStatus(),
      i());
        ...
        (t) => {
          try {
            ((o.qboColumns = (function (t) {
              ...
              return ensureCcAccountColumn(e);
            })(t)),
              b(""));
          } catch (t) {
  ...
  function C() {
    ...
    const ccAccount = qboAccountName(o.selectedAccount);
    n.forEach((row) => { row.ccAccount = ccAccount; });
    a.forEach((row) => { row.ccAccount = ccAccount; });
  ...
  function y() {
    ...
    (t("reconCheckpoint").classList.toggle("pass", a.ok),
      ...
      (function () {
        ...
        const n = w();
        let a =
          '<thead><tr><th>Date</th><th>CC Account</th><th>Payee</th><th>Account</th><th>Description</th><th class="num">Amount</th><th>Memo</th><th>Class</th><th>Flags</th></tr></thead><tbody>';
        (e.imported.forEach((t, e) => {
          ...
          a +=
            ...
            x(t.date) +
            "</td><td>" +
            x(t.ccAccount || qboAccountName(o.selectedAccount)) +
            "</td><td>" +
            A(
              "impSel impPayee",
                ...
                !(function (t) {
                  ...
                  (b(""),
                    ...
                    o.result.imported.push({
                      date: e.date,
                      ccAccount: qboAccountName(o.selectedAccount),
                      payee: n.payee,
                      account: n.account,
        ...
        const e = ExpCore.reconcile(o.bankRows, t.imported, t.excluded),
          a = {
            ...
            imported: t.imported.map((t) => ({
              date: t.date,
              ccAccount: t.ccAccount || qboAccountName(o.selectedAccount),
              payee: t.payee,
              account: t.account,
              amount: t.amount,
              klass: t.klass,
  ...
  async function v() {
    try {
      const t = o.result,
        ...
        n = ExpXlsx.buildWorkbook(
          ...
          { imported: t.imported, excluded: t.excluded },
          {
            ...
            bankAccountQboName: qboAccountName(o.selectedAccount),
            dateLabel: o.dateLabel,
            reconciliation: e,
            vendorMappings: P(),
            fixedVendorMappings: ExpData.FIXED_VENDOR_MAPPINGS || [],
      ...
      ((i.href = l),
        ...
        i.click(),
        (() => {
          const range = ExpCore.dateRange(o.bankRows || []);
          if (range.max) {
            saveImportedTill(o.selectedAccount, range.max);
            renderImportStatus();
          }
        })(),
        setTimeout(() => {
